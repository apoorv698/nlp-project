...
def training(base_path, train_path, json_filename, procname, return_dict):
	_token = {}
	__token = {}
	counter = 0
		...
		try:
			...
			for line in fd:
				...
				for i in range(len(word_token)-1):
					word_token[i] = word_token[i].lower()
					word_token[i+1] = word_token[i+1].lower()
					if word_token[i] not in stopword and regex.match(word_token[i]) is None and word_token[i+1] not in stopword and regex.match(word_token[i+1]) is None:
						word_token[i] = lemmatizer.lemmatize(word_token[i])
						word_token[i+1] = lemmatizer.lemmatize(word_token[i+1])
						# wt_list = []
						_token[word_token[i]+' '+word_token[i+1]]=_token.get(word_token[i]+' '+word_token[i+1],0)+1
						__token[word_token[i]] = __token.get(word_token[i],0)+1
						# for wt in wt_list:
	...
	with open(base_path+json_filename, 'w') as fd:
		json.dump(__token, fd)
	with open(base_path+'_'+json_filename, 'w') as fd:
		json.dump(_token, fd)
	return_dict['_'+procname]= __token
	return_dict[procname]= _token


def testing(test_path, review_test):
	total_count = 0
	_count = 0
	# print(len(pos_token))
	# print(len(neg_token))
	# print(len(pos_token_uni))
	# print(len(neg_token_uni))
	for filename in os.listdir(test_path):
		...
		try:
			...
			for review_line in fd:
				...
				for i in range(len(review_token)-1):
					review_token[i] = review_token[i].lower()
					review_token[i+1] = review_token[i+1].lower()
					if review_token[i] not in stopword and regex.match(review_token[i]) is None and review_token[i+1] not in stopword and regex.match(review_token[i+1]) is None:
						review_token[i] = lemmatizer.lemmatize(review_token[i])
						review_token[i+1] = lemmatizer.lemmatize(review_token[i+1])
						#print(review_token[i]+' '+review_token[i+1])
						#print(pos_token.get(review_token[i]+' '+review_token[i+1],0), neg_token.get(review_token[i]+' '+review_token[i+1],0))
						review_count_pos *= (float(pos_token.get(review_token[i]+' '+review_token[i+1],0)+1.0)/(pos_token_uni.get(review_token[i],0)+len(pos_token)*1.0+len(neg_token)*1.0)*1.0)*1000
						review_count_neg *= (float(neg_token.get(review_token[i]+' '+review_token[i+1],0)+1.0)/(neg_token_uni.get(review_token[i],0)+len(pos_token)*1.0+len(neg_token)*1.0)*1.0)*1000
						# total_rt = pos_token.get(rt,0)+neg_token.get(review_token[i],0)
						# try:
		...
		if review_test == POSITIVE_REVIEW:
			if review_count_pos>review_count_neg:
				_count+=1
		...
		if total_count %500 ==0:
			print(str(total_count)+ ' '+review_test.lower()+' test review sample checked.')
...
def testing_using_userInput():
	while True:
		review_count_pos = 1.0
		review_count_neg = 1.0
		review = input('Enter review: ')
		review_token = nltk.tokenize.word_tokenize(review)
		for sw in stopword:
			if sw in review_token:
				review_token.remove(sw)
		for i in range(len(review_token)-1):
			review_token[i] = review_token[i].lower()
			review_token[i+1] = review_token[i+1].lower()
			if  regex.match(review_token[i]) is None and regex.match(review_token[i+1]) is None:
				review_token[i] = lemmatizer.lemmatize(review_token[i])
				review_token[i+1] = lemmatizer.lemmatize(review_token[i+1])
				# using Laplace Smoothing and normalization
				review_count_pos *= (float(pos_token.get(review_token[i]+' '+review_token[i+1],0)+1.0)/(pos_token_uni.get(review_token[i],0)+len(pos_token)*1.0+len(neg_token)*1.0)*1.0)*1000
				review_count_neg *= (float(neg_token.get(review_token[i]+' '+review_token[i+1],0)+1.0)/(neg_token_uni.get(review_token[i],0)+len(pos_token)*1.0+len(neg_token)*1.0)*1.0)*1000
		logging.debug(review_count_pos, review_count_neg)
		if review_count_pos>review_count_neg:
...
if __name__ == '__main__':
	...
	if os.path.isfile(base_path+'training_pos_result.json') and os.path.isfile(base_path+'training_neg_result.json'):
		print('Using preprocessed data.')
		with open(base_path+'_training_pos_result.json','r') as fd:
			pos_token = json.load(fd)
		with open(base_path+'training_pos_result.json','r') as fd:
			pos_token_uni = json.load(fd)
		with open(base_path+'_training_neg_result.json','r') as fd:
			neg_token = json.load(fd)
		with open(base_path+'training_neg_result.json','r') as fd:
			neg_token_uni = json.load(fd)
	else:
		json_filename ='training_pos_result.json'
		p1=multiprocessing.Process(target=training, args=(base_path, pos_train_path, json_filename, 'p1', return_dict))
		json_filename = 'training_neg_result.json'
		p2=multiprocessing.Process(target=training, args=(base_path, neg_train_path, json_filename, 'p2', return_dict))
		p1.start()
		p2.start()
		p1.join()
		p2.join()
		pos_token = return_dict['_p1']
		neg_token = return_dict['_p2']
		pos_token_uni = return_dict['p1']
		neg_token_uni = return_dict['p2']
	...
	pos_count, total_pos_count = testing(pos_test_path, POSITIVE_REVIEW)
	print("Accuracy with positive test samples is "+str(round(float(1.0*pos_count/total_pos_count*1.0*100),5))+" %")

	total_neg_count = 0
	neg_count = 0

	neg_count, total_neg_count = testing(neg_test_path, NEGATIVE_REVIEW)
